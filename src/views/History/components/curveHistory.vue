<template lang="">
  <div>
    <div class="time">
      查看时间：
      <a-button
        :type="classR === 'LAST_YEAR' ? 'primary' : ''"
        @click="timerUp('LAST_YEAR')"
        >年</a-button
      >
      <a-button
        :type="classR === 'LAST_MONTH' ? 'primary' : ''"
        @click="timerUp('LAST_MONTH')"
        >月</a-button
      >
      <a-button
        :type="classR === 'LAST_WEEK' ? 'primary' : ''"
        @click="timerUp('LAST_WEEK')"
        ref="day"
        >周</a-button
      >
      <a-button
        :type="classR === 'LAST_DAY' ? 'primary' : ''"
        @click="timerUp('LAST_DAY')"
        >日</a-button
      >
      <a-button
        :type="classR === 'LAST_HOUR' ? 'primary' : ''"
        @click="timerUp('LAST_HOUR')"
        >小时</a-button
      >
...
<script>
import { getActualSpeed } from '@/services/history'
export default {
  name: 'CurveEcharts',
  props: {
    fanId: {
  ...
  methods: {
    ...
    async timerUp(h) {
      ...
      const res = await getActualSpeed(this.params)
