  ...
  <div>
    ...
    <a-form  :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
      <a-form-item label="用户名">
        <a-input
          v-decorator="['用户名', { rules: [{ required: true, message: 'Please input your username!' }] }]"
          v-model="name"
        />
      </a-form-item>
      ...
      <a-form-item label="密码">
        <a-input
          v-decorator="['密码', { rules: [{ required: true, message: 'Please input your password!' }] }]"
          v-model="password"
        />
      </a-form-item>
      <a-form-item :wrapper-col="{ span: 12, offset: 5 }">
        <a-button type="primary" html-type="submit" @click="clickHandler">保存</a-button>
        <a-button type="danger" class="reset">重置</a-button>
      </a-form-item>
    </a-form>
  </div>
</template>
